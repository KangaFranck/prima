  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-red-500 font-neue">{error}</p>
      </div>
    );
  }
        ...
        <div className="absolute inset-0 bg-black/30 backdrop-filter">
          ...
          <div className="relative h-full flex items-center justify-center text-center">
            <div className="transform translate-y-8 will-change-transform">
              <h1 className="text-[45px] sm:text-[60px] md:text-[75px] lg:text-[90px] font-neue font-extralight text-white leading-[0.9] tracking-wider uppercase mb-4 [-webkit-font-smoothing:antialiased] [text-rendering:optimizeLegibility]">
                Nos Restaurants
              </h1>
        ...
        <div className="max-w-7xl mx-auto px-4">
          {activeRestaurants.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-xl font-neue text-gray-600">Aucun restaurant n'est disponible pour le moment.</p>
            </div>
          ) : (
            <motion.div 
                    ...
                    <div className="h-[160px] relative flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                      {restaurant.logo ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <span className="text-3xl font-neue font-light text-gray-400">{restaurant.nom.charAt(0)}</span>
                        </div>
                      )}
                    </div>
                    <div className="h-[40px] flex items-center justify-center gap-2">
                      <h3 className="text-center text-gray-900 font-neue font-light text-base group-hover:text-blue-600 transition-colors duration-300 px-3 truncate">
                        {restaurant.nom}
                      </h3>
                      <span className={`text-xs font-neue font-medium px-1.5 py-0.5 rounded ${
                        isCurrentlyOpen({
